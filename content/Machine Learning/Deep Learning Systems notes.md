...
Let's consider the matrix form of our updates, the layer normalization equals to normalizing the _rows_ of the matrix, and when we try to normalize the _columns_ of the matrix, we get batch normalization. As we are normalizing over the mini batch.
...
A common solution is use the batch norm on _training_ time, but when _testing_, compute a _running average_ of mean/variance for all features at each layer and normalize by them.
